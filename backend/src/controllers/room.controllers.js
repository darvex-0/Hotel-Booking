...
exports.createRoom = async (req, res) => {
  try {
    ...
    const data = {
      ...
      room_size,
      room_capacity,
      allow_pets: String(allow_pets) === 'true',
...
exports.getRoomByIdOrSlugName = async (req, res) => {
  try {
    ...
    // If not found by ID or ID is not numeric, try finding by slug
    if (!room) {
...
exports.editRoomByAdmin = async (req, res) => {
  try {
    ...
    // Images are now optional in Edit mode
    ...
    if (req.files && req.files.length > 0) {
      ...
      updateData.room_images = req.files.map(
